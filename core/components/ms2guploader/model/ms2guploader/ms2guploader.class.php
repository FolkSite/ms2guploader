<?php
/**
 * The base class for ms2guploader.
 *
 * @package ms2guploader
 */
class ms2guploader
{
  /* @var modX $modx */
   ...
   * @return array
   */
  public function initialize($ctx = 'web', $currentFiles)
  {
    if (!$this->pdoTools) {
      $this->loadPdoTools();
    }
    ...
    $_SESSION['ms2guploader'][$hash] = $this->config;
    ...
    $config_js = preg_replace(array('/^\n/', '/\t{6}/'), '', '
        ms2guploaderConfig = {
          ...
          ,currentFiles: ' . $this->config['currentFiles'] . '
        };
      ');
  ...
  public function fileUpload($data)
  {
    if (!$this->authenticated || empty($this->config['allowFiles'])) {
      return $this->error('ms2guploader_err_access_denied');
    }
	//$ms2Gallery = $this->modx->getService('ms2gallery', 'ms2Gallery', MODX_CORE_PATH . 'components/ms2gallery/model/ms2gallery/');
	...
	if(empty($file['thumb'])) $file['thumb'] = '/assets/images/resources/'.$file['path'].$this->config['thumbSize'].'/'.$file['file'];
    $html = $this->getChunk( $this->config['tpl'], $file);
    return $this->success("ok", ['html' => $html, 'file' => $file]);
  }



  public function fileLimit($data){
  	//$this->config['uploadLimit']
  ...
  public function productSave(array $data)
  {
    ...
    if (!empty($errors)) {
      return $this->error($this->modx->lexicon('ms2guploader_err_form'), $errors);
    }
    ...
    if ($response->isError()) {
      ...
      if (empty($message)) {
        $message = $this->modx->lexicon('ms2guploader_err_form');
      }
    ...
    if($data['redirectPublished'] == '0'){
      $successMessage = 'ms2guploader_published';
  ...
  public function sendMail($uid, $subject, $body)
  {
    ...
    $mail->set(modMail::MAIL_SENDER, $this->modx->getOption('ms2guploader.mail_from', null, $this->modx->getOption('emailsender'), true));
    $mail->set(modMail::MAIL_FROM, $this->modx->getOption('ms2guploader.mail_from', null, $this->modx->getOption('emailsender'), true));
    $mail->set(modMail::MAIL_FROM_NAME, $this->modx->getOption('ms2guploader.mail_from_name', null, $this->modx->getOption('site_name'), true));
